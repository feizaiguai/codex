#!/usr/bin/env python3
"""
Weibo Trending Analyzer - 微博热搜分析器
...
@dataclass
class WeiboTrendingConfig:
    """配置参数"""
    api_key: str = ""
    api_url: str = "https://apis.tianapi.com/weibohot/index"
    limit: int = 10
...
class WeiboTrendingAnalyzer:
    """微博热搜分析器核心类"""

    def __init__(self, config: WeiboTrendingConfig = None):
        """初始化分析器"""
        self.config = config or WeiboTrendingConfig()
    ...
    def fetch_trending(self) -> List[Dict]:
        ...
        for attempt in range(self.config.max_retries):
            try:
                print(f"📡 正在获取微博热搜... (尝试 {attempt + 1}/{self.config.max_retries})")
                ...
                if data.get('code') == 200:
                    ...
                    self.update_time = result.get('update_time', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    ...
    def parse_topics(self, raw_data: List[Dict]) -> List[TrendingTopic]:
        ...
        for idx, item in enumerate(raw_data, 1):
            try:
                ...
                # 解析热度值（可能包含分类前缀，如"晚会 61506"）
                hotwordnum = item.get('hotwordnum', '0').strip()
                # 移除可能的分类前缀，只保留数字
                parts = hotwordnum.split()
                if parts:
                    hot_value_str = ''.join(c for c in parts[-1] if c.isdigit())
                    hot_index = int(hot_value_str) if hot_value_str else 0
                else:
                    hot_index = 0
                ...
                print(f"⚠️ 解析话题失败: {item.get('hotword', 'Unknown')} - {str(e)}")
                continue
    ...
    def generate_report(self, topics: List[TrendingTopic]) -> str:
        ...
        for topic in topics:
            ...
            report_lines.append(f"### {topic.rank}. {topic.title} {tag_emoji}")
            report_lines.append(f"")
            report_lines.append(f"**热搜指数**: {topic.hot_index:,}")
            ...
            # 微博链接
            report_lines.append(f"**🔗 微博链接**: [{topic.title}]({topic.url})")
            report_lines.append(f"")
            report_lines.append(f"---")
            report_lines.append(f"")
        ...
        report_lines.append(f"")
        report_lines.append(f"- **数据源**: 天行API (微博官方热搜榜)")
        report_lines.append(f"- **更新频率**: 每30分钟")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='微博热搜分析器')
    ...
    # 执行分析
    analyzer = WeiboTrendingAnalyzer(config)
    ...
    # 保存到文件
    output_file = f"weibo_trending_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
