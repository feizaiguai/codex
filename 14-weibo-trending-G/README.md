...
自动抓取微博实时热搜榜单，并为每个热搜话题搜索详细的新闻背景和上下文信息。

## ✨ 核心功能

- 🔥 **实时热搜抓取** - 通过天行API获取微博实时热搜榜单
- 🔍 **智能话题分析** - 使用WebSearch为每个话题搜索背景信息
- 📊 **热度指标** - 提供热搜指数、排名、话题分类等数据
- 📝 **深度解读** - 自动生成话题摘要、关键信息和事件时间线
- 🎯 **自定义筛选** - 支持按排名、分类、关键词筛选
...
# 获取默认前10名热搜
python 14-weibo-trending/handler.py

# 获取前5名
python 14-weibo-trending/handler.py --limit 5

# 搜索关键词
python 14-weibo-trending/handler.py --keyword "AI"

# 只要基本信息（不包含详细分析）
python 14-weibo-trending/handler.py --no-analysis
```
...
```python
from handler import WeiboTrendingConfig, WeiboTrendingAnalyzer
...
- **API接口**: https://apis.tianapi.com/weibohot/index
- **数据源**: 微博官方热搜榜
- **更新频率**: 每30分钟
- **数据时效**: 实时（有30分钟延迟）
...
#### 选项1: FreeAPIs.cn
- **API接口**: https://api.freeapis.cn/v1/weibo/hot
- **注册地址**: https://www.freeapis.cn/user/key
- **配额**: 10,000次/天，1次/秒
- **状态**: ⚠️ 需要注册获取KEY

#### 选项2: ALAPI
- **API接口**: https://v2.alapi.cn/api/weibo/hot
...
```bash
# 测试基本功能（不含详细分析）
python 14-weibo-trending/handler.py --limit 3 --no-analysis

# 测试完整功能（包含15-web-search）
python 14-weibo-trending/handler.py --limit 2
```
