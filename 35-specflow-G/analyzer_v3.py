...
class SpecificationAnalyzer:
    ...
    def analyze(self, spec: SpecificationDocument) -> QualityReport:
        ...
        Returns:
            QualityReport: 包含质量指标,验证问题和改进建议的质量报告
        """
        # 1. 计算质量指标
    ...
    def _calculate_quality_metrics(self, spec: SpecificationDocument) -> QualityMetrics:
        ...
        评分规则:
        - 完整性:检查必要文档,需求,用户故事,验收标准是否齐全
        - 一致性:检查术语,优先级,依赖关系是否一致
        - 原子性:评估需求是否足够细化和独立
        - 可测试性:评估验收标准和测试用例的质量
        ...
        metrics = QualityMetrics(
            ...
            overall_grade=QualityGrade.C  # 临时值,稍后计算
        )
    ...
    def _calculate_completeness(self, spec: SpecificationDocument) -> float:
        ...
        # 1. 项目基本信息 (10分)
        if spec.project_name and spec.project_name != "未命名项目":
            score += 10
    ...
    def _calculate_atomicity(self, spec: SpecificationDocument) -> float:
        ...
        if spec.user_stories:
            ...
            for story in spec.user_stories:
                # 检查 i_want 和 so_that 是否简洁(不超过100字符)
        ...
        # 4. 没有复合需求 (20分)
        # 检查描述中是否有"和","以及","同时"等复合词
        if spec.requirements:
            atomic_count = 0
            compound_keywords = ['和', '以及', '同时', '并且', '以及', '还要', '另外']
    ...
    def _calculate_testability(self, spec: SpecificationDocument) -> float:
        ...
        # 2. 有明确的输入输出 (25分)
        # 检查验收标准中是否提到输入,输出,Given,When,Then
        io_keywords = ['输入', '输出', 'Given', 'When', 'Then', '前提', '操作', '结果']
    ...
    def _collect_validation_issues(self, spec: SpecificationDocument) -> List[ValidationIssue]:
        ...
        # 1. 检查项目基本信息
        if not spec.project_name or spec.project_name == "未命名项目":
            issues.append(ValidationIssue(
                rule_id="R001",
                severity=Severity.HIGH,
                description="项目名称缺失或使用默认值",
                location="项目信息",
                suggestion="请提供明确的项目名称"
            ))
        ...
        if not spec.requirements or len(spec.requirements) < 3:
            issues.append(ValidationIssue(
                ...
                description=f"需求数量偏少(当前{len(spec.requirements)}个)",
                location="需求列表",
                suggestion="建议至少定义3个功能需求"
            ))
        ...
        if not spec.user_stories or len(spec.user_stories) < 5:
            issues.append(ValidationIssue(
                rule_id="R003",
                severity=Severity.MEDIUM,
                description=f"用户故事数量偏少(当前{len(spec.user_stories)}个)",
                location="用户故事",
                suggestion="建议至少定义5个用户故事"
            ))
        ...
        if spec.user_stories:
            ...
            if stories_without_ac:
                issues.append(ValidationIssue(
                    rule_id="R004",
                    severity=Severity.MEDIUM,
                    description=f"{len(stories_without_ac)}个用户故事缺少验收标准",
                    location=f"用户故事验收标准",
                    suggestion=f"建议为以下故事添加验收标准: {', '.join(stories_without_ac[:3])}"
                ))
        ...
        if spec.requirements:
            ...
            if invalid_req_ids:
                issues.append(ValidationIssue(
                    ...
                    description=f"{len(invalid_req_ids)}个需求ID格式不标准",
                    location="需求ID格式",
                    suggestion="建议使用 REQ-xxx,FR-xxx 或 NFR-xxx 格式"
                ))
        ...
        if spec.user_stories:
            ...
            if invalid_story_ids:
                issues.append(ValidationIssue(
                    rule_id="R006",
                    severity=Severity.LOW,
                    description=f"{len(invalid_story_ids)}个用户故事ID格式不标准",
                    location="用户故事ID格式",
                    suggestion="建议使用 US-xxx 格式"
                ))
                ...
                if req_deps:
                    ...
                    if invalid_deps:
                        issues.append(ValidationIssue(
                            rule_id="R007",
                            severity=Severity.HIGH,
                            description=f"依赖的需求ID不存在: {', '.join(invalid_deps)}",
                            location=f"需求 {req.id}",
                            suggestion="请检查依赖关系或添加缺失的需求"
                        ))
    ...
    def _generate_recommendations(
        ...
        if metrics.completeness_score < 60:
            recommendations.append(
                "📋 **完整性较低**:建议补充缺失的文档和需求信息,确保项目信息,需求,用户故事等核心内容齐全."
            )
        elif metrics.completeness_score < 80:
            recommendations.append(
                "📋 **完整性中等**:大部分内容已齐全,建议补充验收标准和测试用例以提升质量."
            )

        # 2. 基于一致性得分
        if metrics.consistency_score < 60:
            recommendations.append(
                "🔄 **一致性较低**:建议检查需求ID,用户故事ID的唯一性和格式统一性,确保依赖关系有效."
            )
        elif metrics.consistency_score < 80:
            recommendations.append(
                "🔄 **一致性中等**:大部分内容一致,建议统一术语使用和优先级分布."
            )

        # 3. 基于原子性得分
        if metrics.atomicity_score < 60:
            recommendations.append(
                "⚛️ **原子性较低**:建议将复合需求拆分为独立的小需求,确保每个需求和用户故事职责单一."
            )
        elif metrics.atomicity_score < 80:
            recommendations.append(
                "⚛️ **原子性中等**:大部分需求已足够细化,建议优化验收标准使其更加明确."
            )

        # 4. 基于可测试性得分
        if metrics.testability_score < 60:
            recommendations.append(
                "🧪 **可测试性较低**:建议为每个用户故事添加明确的,可量化的验收标准,补充测试用例和测试策略."
            )
        elif metrics.testability_score < 80:
            recommendations.append(
                "🧪 **可测试性中等**:大部分内容可测试,建议增强验收标准的可量化性(如响应时间,错误率等)."
            )
        ...
        if high_count > 0:
            recommendations.append(
                f" **发现{high_count}个严重问题**:请优先修复高优先级问题(如无效的依赖关系,缺失的必要信息)."
            )

        if medium_count > 3:
            recommendations.append(
                f" **发现{medium_count}个警告**:建议逐步解决中等优先级问题以提升文档质量."
            )
        ...
        if avg_score >= 90:
            recommendations.append(
                "🌟 **质量优秀**:文档质量非常高,可以直接进入开发阶段.建议定期回顾和更新文档."
            )
        elif avg_score >= 75:
            recommendations.append(
                " **质量良好**:文档质量较好,可以开始开发.建议在开发过程中持续完善文档."
            )
        elif avg_score >= 60:
            recommendations.append(
                " **质量中等**:文档基本可用,但建议先解决主要问题后再开始开发,避免后期返工."
            )
        else:
            recommendations.append(
                " **质量偏低**:建议先完善文档,补充缺失内容,解决关键问题后再开始开发."
            )

        # 7. 特定领域建议
        if hasattr(spec, 'domain'):
            domain_tips = {
                DomainCategory.FINTECH: "💰 金融领域项目,建议特别关注安全性,合规性和数据一致性需求.",
                DomainCategory.HEALTHCARE: "🏥 医疗领域项目,建议关注患者隐私,数据安全和医疗标准合规性.",
                DomainCategory.E_COMMERCE: "🛒 电商领域项目,建议关注支付安全,库存管理和用户体验优化.",
                DomainCategory.ENTERPRISE: "🏢 企业应用项目,建议关注系统集成,权限管理和可扩展性."
            }
    ...
    def _estimate_hours(self, spec: SpecificationDocument) -> int:
        ...
        - 每个需求:+5小时
        - 每个用户故事:+8小时
        - 复杂度调整:Simple 0.8x, Medium 1.0x, Complex 1.3x, Very Complex 1.6x
        - 文档完整度调整:根据文档数量调整
        ...
        if doc_count >= 7:
            total_hours = int(total_hours * 0.9)  # 文档齐全,减少10%工时
        elif doc_count <= 3:
            total_hours = int(total_hours * 1.2)  # 文档不全,增加20%工时

        return max(total_hours, 20)  # 最少20小时
